...
ax00.plot(t, two_fidelity[0], color = cmap(3/4), label=r'$D(\alpha) = {disc}$'.format(disc=discord[0]))
ax00.set_ylabel(r"Fidelity")
ax01.plot(t, two_fidelity[-1], color = cmap(1/4), label=r'$D(\alpha) = {disc}$'.format(disc=discord[-1]))
ax10.plot(t, one_fidelity_0, color = cmap(3/4), label=r'$D(\alpha) = {disc}$'.format(disc=discord[0]))
ax10.set_xlabel(r'$t$')
ax10.set_ylabel(r"Fidelity")
ax11.plot(t, one_fidelity_1, color = cmap(1/4), label=r'$D(\alpha) = {disc}$'.format(disc=discord[-1]))
ax11.set_xlabel(r'$t$')
